...
class CgalConan(ConanFile):
    ...
    def layout(self):
        # use current working directory as root folder
        src_folder = os.environ.get("SETUP_EXT__CONAN_SOURCE_FOLDER__cgal", None)
        if src_folder is not None:
            src_folder = os.path.normpath(os.path.abspath(src_folder))
        else:
            src_folder = "src"
    ...
    def package(self):
        ...
        self._create_cmake_module_variables(os.path.join(self.package_folder, self._cmake_module_file_rel_path))
